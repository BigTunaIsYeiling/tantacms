...
import { Filters, FilterDivision } from "@/Lib/FiltersSlices/CoursesSlice";
...
export default function Scelection({ data }) {
  ...
  const names = data.flatMap((course) =>
    course.divisions.map((division) => division.name)
  );
  //    [
  //     ...new Set(data.map((opt) => opt.divisions?.map((div) => div.name))),
  //   ];
